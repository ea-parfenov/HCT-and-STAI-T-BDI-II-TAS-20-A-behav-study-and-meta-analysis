# HCT and BDI-II ====
# Description: script to conduct meta-analysis investigating relationships between the HCT and BDI-II
...
mydata = read.table("Articles - BDI-II.tsv", sep = '\t', quote = "", header=TRUE)
...
# Set levels in the order they appear in the data
...
p3 <- p3 + theme(plot.margin = unit(c(0.2, 0.7, 0, 1), "lines"))

p4 <- grid.arrange(p1, p2, p3, nrow = 1, widths=c(2,1.25,1.25))

ggsave('influence-analysis.png', path='plots/BDI', plot=p4, width=10, height=4, dpi=600)
...
png(filename='plots/BDI/forest-plot_all.png', res=600, width=6, height=8, units='in')
...
legend("bottomright", legend = names(colour_palette), fill = colour_palette, title = "Population type", xpd = TRUE, 
       ...
       y.intersp = 0.8, # adjust vertical interspacing
       cex = 0.6) # adjust font size
par(xpd = TRUE)
text(-2, res$k+2, "Total", cex = 0.5, font = 2)
text(-2, -1, paste(sum(mydata$N_sub)), cex = 0.5, font = 2)
...
## Moderation analysis ====
### Population type ====
mod_part <- rma.mv(Corrected_r, Variance, random = ~ 1 | Article, mods = ~ Participants, data = mydata,
                   tdist = TRUE # base test statistics and confidence intervals on the t-distribution
)
summary(mod_part)

# Plot and save
mod_partic <- ggplot(mydata, aes(x = Participants, y = Corrected_r, size = 1/Variance)) + 
  geom_point(data = mydata, shape = 1) + 
  ...
  scale_y_continuous(breaks = round(seq(min(mydata$Corrected_r), max(mydata$Corrected_r), by = 0.2), 1))
ggsave('participants-moderator.png', path='plots/BDI', plot=mod_partic, width=4, height=6, dpi=600)

### Instructions used ====
# Remove one study which didn't report type of instructions used
mydata_instr <- mydata[!is.na(mydata$Instructions_comb),]
mod_instr <- rma.mv(Corrected_r, Variance, random = ~ 1 | Article, mods = ~ Instructions_comb, data = mydata_instr, 
                    tdist = TRUE # base test statistics and confidence intervals on the t-distribution
)
summary(mod_instr)

# Plot and save
mod_instruct <- ggplot(mydata_instr, aes(x = Instructions_comb, y = Corrected_r, size = 1/Variance))+
  geom_point(data = mydata_instr, shape = 1) +
  xlab("") + ylab("") +
  geom_hline(yintercept = 0, linetype="dashed") +
...
ggsave('instructions-moderator.png', path='plots/BDI', plot=mod_instruct, width=4, height=6, dpi=600)


### Formula used ====
mod_formula <- rma.mv(Corrected_r, Variance, random = ~ 1 | Article, mods = ~ Formula_type, data = mydata,
                      tdist = TRUE # base test statistics and confidence intervals on the t-distribution
)
summary(mod_formula)

# Plot and save
mod_form <- ggplot(mydata, aes(x = Formula_type, y = Corrected_r, size = 1/Variance)) + 
  geom_point(data = mydata, shape = 1) + 
  xlab("") + ylab("") +
  geom_hline(yintercept = 0, linetype="dashed") +
  theme(axis.text.y = element_text(size = 10, colour = 'black'),
...
ggsave('formula-moderator.png', path='plots/BDI', plot=mod_form, width=4, height=6, dpi=600)
...
reg <- regtest(Corrected_r, Variance, data=mydata)
reg

### Funnel plot ====
png(filename='plots/BDI/funnel_all.png', res=600, width=6, height=6, units='in')
...
mydata$sei <- sqrt(mydata$Variance)
mydata$yi <- mydata$Corrected_r
power_parf <- mapower_se(dat=mydata, 
                         observed_es = -0.05, # observed effect size from the model
...
### Sunset plot ====
sunset_plot <- viz_sunset(res, true_effect = -0.05, sig_level = 0.05, power_contours = "continuous", xlab = "Observed correlation")
ggsave('plots/BDI/sunset.png', plot = sunset_plot, dpi = 600, width = 6, height = 6, units = 'in')
