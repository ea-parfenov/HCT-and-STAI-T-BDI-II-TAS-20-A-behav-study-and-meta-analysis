# HCT and TAS-20 ====
# Description: script to conduct meta-analysis investigating relationships between the HCT and TAS-20
...
mydata = read.table("Articles - TAS-20.tsv", sep = '\t', quote = "", header=TRUE)
...
# Set levels in the order they appear in the dataset
...
p1 <- qplot(cook_vals, ID, data=mydata) + 
  geom_point(data = mydata[c(2, 23), ], aes(color = "red")) +
...
p4 <- grid.arrange(p1, p2, p3, nrow = 1, widths=c(1.5,1,1))

ggsave('influence-analysis.png', path='plots/TAS', plot=p4, width=10, height=4, dpi=600)

# Remove influential cases
data_inf = mydata[-c(2, 23), ]
...
png(filename='plots/TAS/forest-plot_all.png', res=600, width=6, height=8, units='in')
...
ggsave('participants-moderator.png', path='plots/TAS', plot=mod_partic, width=4, height=6, dpi=600)

### Instructions used ====
mod_instr <- rma.mv(Corrected_r, Variance, random = ~ 1 | Article, mods = ~ Instructions_comb, data = data_inf, 
                    tdist = TRUE # base test statistics and confidence intervals on the t-distribution
)
summary(mod_instr)

# Plot and save
mod_instruct <- ggplot(data_inf, aes(x = Instructions_comb, y = Corrected_r, size = 1/Variance)) + 
  geom_point(data = data_inf, shape = 1) + 
  xlab("") + ylab("") +
  geom_hline(yintercept = 0, linetype="dashed") +
  theme(axis.title = element_text(size = 15),
...
ggsave('instructions-moderator.png', path='plots/TAS', plot=mod_instruct, width=4, height=6, dpi=600)
...
ggsave('formula-moderator.png', path='plots/TAS', plot=mod_form, width=4, height=6, dpi=600)
...
### Funnel plot ====
png(filename='plots/TAS/funnel_all.png', res=600, width=6, height=6, units='in')
...
power_parf <- mapower_se(dat=data_inf, 
                         observed_es = -0.07, # observed effect size from the model
...
### Sunset plot ====
sunset_plot <- viz_sunset(res, true_effect = -0.07, sig_level = 0.05, power_contours = "continuous", xlab = "Observed correlation")
ggsave('plots/TAS/sunset.png', plot = sunset_plot, dpi = 600, width = 6, height = 6, units = 'in')
